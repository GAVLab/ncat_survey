...
survey_stripes_dir = os.path.join(os.path.dirname(__file__), '..', 'survey', 'stripes')
stripe_file_names = [os.path.join(survey_stripes_dir, name) for name in ('inner.txt', 'middle.txt', 'outer.txt')]
...
# 
data = []
for stripe_file_name in stripe_file_names:
  pts = []
  with open(stripe_file_name, 'r') as file:
    for line in file:
